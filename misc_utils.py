...
def create_Be(u_0,params):
    ...
    nx,nt,tau = params['nx'],params['nt'],params['tau']

    ye0 = np.zeros(int(2*nx**tau))
    sizes = [int(nx**j) for j in range(1,tau+1)]
    sizes.insert(0,0)
    delta = np.sum(sizes)
    offset = int(2*nx**tau-delta)
    tmp = np.array([1])
    for j in range(1,tau+1):
        tmp = np.kron(tmp,u_0)
        ye0[offset+np.sum(sizes[:j]):offset+np.sum(sizes[:j+1])] = tmp

    Be = np.zeros(int(2*nt*nx**tau))
    Be[:int(2*nx**tau)] = ye0
...
def normalized_initial_condition(params):
    ...
    Returns:    
    """
    nx,nt,tau,Length,ic_type = params['nx'],params['nt'],params['tau'],params['Length'],params['ic_type']
    sig  = 0.5
    mu   = np.pi

    if (ic_type=='Gaussian'):
        if (nx<2**10 and nt<2**10 and tau<4): #slow for large circuits
...
def solve_sys(Le_real,Be,params):
    ...
    delta = np.sum([int(nx**j) for j in range(1,tau+1)])
    for i in range(nt):
        offset = int((i+1)*2*nx**tau - delta)
